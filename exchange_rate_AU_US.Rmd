...
data.frame('номер_переменной'= seq(3,22,1),'переменная'=colnames(data_norm)[3:22],'p_value' = na.omit(cor_p)[1:20], "значимость" = na.omit(cor_p)[1:20]<0.)
```  
...
```
Вероятность ошибиться, считая что кросс- и авто- корреляция отсутствует 0.36-0.45. Достаточно высока! Скорее всего мы ошибаемся. Проведем сравнение с ARIMA:
